...
def obtener_datos_final_mundo():
    ...
    datos_partido = {"detalles": {}, "eventos": [], "jugadores": []}
    try:
        ...
        if res.status_code == 200:
            ...
            if "response" in datos_json and len(datos_json["response"]) > 0:
                ...
                # Extrayendo estadísticas de jugadores
                if "players" in partido:
                    for equipo in partido["players"]:
...
async def obtener_trivias():
    ...
    try:
        ...
        headers_grok = {"Authorization": f"Bearer {GROK_API_KEY}", "Content-Type": "application/json"}
        
        payload = {
            "model": "grok-beta",
            "messages": [
                {"role": "system", "content": "Responde solo con un JSON válido que contenga la clave 'preguntas' con un array de 12 objetos: pregunta, opciones (3), correcta. Basate en los datos técnicos del partido proporcionados."},
                {"role": "user", "content": f"Genera una trivia sobre la final del mundial Qatar 2022 usando estos datos: {json.dumps(contexto)}"}
            ],
            "temperature": 0.7
        }
